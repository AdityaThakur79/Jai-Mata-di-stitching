...
const WomensWear = () => {
  ...
  // Fetch women's items
  const { data, isLoading, isError } = useGetAllItemMastersQuery({ page, limit, search, category: 'women' });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-amber-50 pt-20 relative">
      {/* Overlays */}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-8">
          ...
          <div className="flex items-center space-x-4">
            <button
              ...
              className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-rose-400 text-white' : 'bg-white text-gray-600 hover:bg-rose-100'}`}
            >
              <Grid size={20} />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-rose-400 text-white' : 'bg-white text-gray-600 hover:bg-rose-100'}`}
            >
              <List size={20} />
            </button>
            <select
              ...
              className="py-2 px-3 border border-gray-300 rounded-lg focus:border-rose-400 focus:ring-rose-400"
            >
              {[8, 16, 24, 32].map(n => (
        ...
        {/* Main Content */}
        <div className={`grid gap-4 ${viewMode === 'grid' ? 'grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4' : 'grid-cols-1'} md:gap-8`}>
          {isLoading ? (
            Array.from({ length: limit }).map((_, idx) => (
              <div key={idx} className="animate-pulse bg-white rounded-lg shadow-md h-72" />
            ))
          ) : isError ? (
            ...
            items.map((item, index) => (
              <BlurFade key={item._id} delay={0.2 + index * 0.07}>
                <div className={`group relative bg-white shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 overflow-hidden ${viewMode === 'list' ? 'flex items-center space-x-6 p-6' : ''}`}
                  style={{ animationDelay: `${index * 100}ms`, animation: 'fadeInUp 0.6s ease-out forwards' }}
                >
                  {/* Eye Icon Button for modal */}
                  <button
                    className="absolute top-3 right-3 z-10 p-1 bg-transparent border-none shadow-none focus:outline-none hover:text-rose-500 transition-colors"
                    style={{ border: 'none', background: 'none', borderRadius: 0 }}
                    onClick={e => {
                      e.stopPropagation();
                      setModalImages([item.itemImage, item.secondaryItemImage].filter(Boolean));
                      setActiveModalIdx(0);
                      setModalImage(true);
                    }}
                    aria-label="View larger image"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="text-gray-400 group-hover:text-rose-500 transition-colors" width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                  </button>
                  {/* Image Container */}
                  <div className={`relative overflow-hidden ${viewMode === 'list' ? 'w-48 h-32' : 'w-full aspect-[3/4]'}`}
                  ...
                  >
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-rose-200/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                  </div>
                  {/* Content */}
                  <div className={`${viewMode === 'list' ? 'flex-1' : 'p-5'}`}>
                    <div className="mb-3">
                      <h3 className="text-lg font-medium font-serif mb-1 group-hover:text-rose-500 transition-colors">
                        {item.name}
                      </h3>
                    </div>
                    <div className="flex items-center space-x-3">
            ...
            <div className="flex sm:flex-col flex-row gap-2 sm:mr-3 mb-2 sm:mb-0 z-10">
              {modalImages.map((img, idx) => (
                <img
                  ...
                  src={img}
                  alt={`Thumbnail ${idx+1}`}
                  className={`w-14 h-14 object-cover rounded-lg border-2 cursor-pointer transition-all duration-200 ${activeModalIdx === idx ? 'border-rose-400 shadow-lg' : 'border-gray-200 opacity-80 hover:opacity-100'}`}
