...
import { useGetCategoryByIdMutation, useUpdateCategoryMutation } from '@/features/api/categoriesApi';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Button } from '@/components/ui/button';
import { Loader2, ArrowLeft, Image as ImageIcon, Info, ListOrdered, Users, Save } from 'lucide-react';
import toast from 'react-hot-toast';

const UpdateCategory = () => {
  const { id } = useParams();
  const [getCategoryById, { isLoading: isLoadingData }] = useGetCategoryByIdMutation();
    ...
    async function fetchCategory() {
      ...
      if (data?.category) {
        setForm({
          ...
          shortDesc: data.category.shortDesc || '',
          features: (data.category.features || []).join(', '),
          startingFrom: data.category.startingFrom?.toString() || '',
          category: data.category.category || '',
          order: data.category.order?.toString() || '1'
        });
        setPrimaryPreview(data.category.categoryPrimaryImage || '');
        setSecondaryPreview(data.category.categorySecondaryImage || '');
      }
    }
    fetchCategory();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.title.trim() || !form.startingFrom || !form.category || (!primaryImageFile && !primaryPreview)) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-white dark:from-gray-900 dark:to-gray-800">
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <div className="mb-8 flex items-center space-x-4">
          <Button variant="ghost" onClick={() => navigate('/employee/website/category')} className="p-2">
            <ArrowLeft className="w-5 h-5" />
          </Button>
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit Category/Item</h1>
            <p className="text-gray-600 dark:text-gray-300 mt-1">Update category/item details</p>
          </div>
        </div>
