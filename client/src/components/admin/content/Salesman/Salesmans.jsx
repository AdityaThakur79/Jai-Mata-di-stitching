...
const Salesmans = () => {
  ...
  const { data, isLoading, refetch } = useGetAllSalesmenQuery({
    page: currentPage,
    limit,
    search: debouncedSearch,
  });

  const [deleteSalesman, { isSuccess, isError }] = useDeleteSalesmanMutation();
  const [getSalesmanById] = useGetSalesmanByIdMutation();

  const [open, setOpen] = useState(false);
  const [selectedSalesman, setSelectedSalesman] = useState(null);

  const handleView = async (salesmanId) => {
    setOpen(true);
    try {
      const { data } = await getSalesmanById(salesmanId);
      if (data?.success) {
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody>
              {isLoading ? (
                ...
                data.salesmen.map((salesman, index) => (
                  <tr key={salesman._id} className="text-left border-t">
                    ...
                    <td className="px-6 py-4">{salesman.name}</td>
                    <td className="px-6 py-4">{salesman.email}</td>
                    <td className="px-6 py-4">
                      <span
                    ...
                    <td className="px-6 py-4 flex gap-2 items-center">
                      <Button
                        ...
                        onClick={() => handleView(salesman._id)}
                      >
                ...
                <tr>
                  <td colSpan="6" className="text-center py-10">
                    ...
                    <p className="text-gray-500 text-sm">No salesmen found</p>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          title={
            <div className="flex flex-col items-center">
              {selectedSalesman?.photoUrl ? (
                ...
                <div className="w-24 h-24 rounded-full bg-gray-300 flex items-center justify-center font-bold text-white text-xl mb-2">
                  {selectedSalesman?.name?.charAt(0).toUpperCase() || "S"}
                </div>
              )}
              <span className="font-semibold text-lg">
                {selectedSalesman?.name || "Salesman Info"}
              </span>
            </div>
          }
          placement="right"
              ...
              <div className="flex flex-col gap-1">
                ...
                <span
                  className={`px-2 py-1 text-xs rounded-md w-fit ${
                    selectedSalesman.status
                      ? "bg-green-100 text-green-700"
                      : "bg-red-100 text-red-700"
                  }`}
                >
                  {selectedSalesman.status ? "Active" : "Inactive"}
                </span>
              </div>
            </div>
          ) : (
